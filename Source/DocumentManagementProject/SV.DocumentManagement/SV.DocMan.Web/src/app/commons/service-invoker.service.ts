...
export class ServiceInvokerService {
  ...
  get(element: any, url): Observable<any> {
    ...
    return this.http.get(url, httpPackage)
      .pipe(
        tap(data => this.log(data.toString())),
        catchError(this.handleError<any>())
      );
  }

  /* Get array */
  public getArray<T>(successMessage: string, errorMessage: string): Observable<T[]> {
    return this.http.get<T[]>(AppSettings.API_ADDRESS)
      .pipe(
        tap(data => this.log(successMessage)),
